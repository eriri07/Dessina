...
public class Test_Class : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Debug.Log("나이 : " + junhee.age);         //클래스가 참조타입이라 junhee_1의 age(나이)를 바꿔도 junhee변수 안에 있는 age라는 값이 바뀐다
        Debug.Log("나이 : " + junhee_1.age);
    }
...
public class Test_Human2 : Human
{
    void Test_method()
    {
        test_protected = "테스트용 문자열";     //protected로 선언이 돼있어서 외부에선 저븐이 불가능하지만 상속받은 클래스에선 접근이 가능하다
    }
